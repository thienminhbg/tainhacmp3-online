...
set -e
sudo apt update
sudo apt install -y python3 python3-venv python3-pip ffmpeg git golang nodejs npm
python3 -m venv .venv
...
pip install -U scdl
echo 'Install complete. Zing uses the Node.js API worker; NCT uses pum; SoundCloud uses scdl.'
echo 'Run ./run.sh'
